...
def process_city_step(message):
    ...
    msg = bot.send_message(message.chat.id, 'Введите название города (не Российского), в котором необходимо найти '
                                            'отель(-и)\nНазвание города можно вводить как на русском так и на '
                                            'английском языках, а так же не имеет значения регистр.')
...
def distance_min(message):
    ...
    if max_price.isdigit() and result[0] < int(max_price):
        ...
        msg = bot.send_message(message.chat.id, 'Минимальное расстояние до центра в км (100 м. указывайте как 0.1, '
                                                'а 1 км можно указать просто "1"): ')
...
def distance_max(message):
    ...
    try:
        if min_distance.isdigit() or float(min_distance):
            ...
            msg = bot.send_message(message.chat.id, 'Максимальное расстояние до центра в км (100 м. указывайте как '
                                                    '0.1, а 1 км можно указать просто "1"): ')
            bot.register_next_step_handler(msg, process_hotels_step)
        else:
...
def process_hotels_step(message):
    ...
    if result[0] != '/bestdeal':
        ...
        if city.isalpha():
            ...
            msg = bot.send_message(message.chat.id, 'Сколько отелей Вам найти (не более 3)?')
        ...
        logging.info(f'Пользователь: {user_name} - id: {user_id} - задал максимальное расстояние до центра:'
                     f' {max_distance} км.')
        try:
            if max_distance.isdigit() or float(max_distance):
                ...
                msg = bot.send_message(message.chat.id, 'Ошибка при вводе! введите максимальное расстояние до центра '
                                                        'в км (100 м. указывайте как 0.1, а 1 км можно указать просто'
                                                        ' "1"): ')
                bot.register_next_step_handler(msg, process_hotels_step)
        except ValueError:
...
def calendar1(call):
    ...
    result_low, key, step = DetailedTelegramCalendar(locale='ru', calendar_id=0, min_date=date.today()).process(call.data)
    if not result_low and key:
        bot.edit_message_text('Дата', call.message.chat.id, call.message.message_id, reply_markup=key)
    elif result_low:
        bot.edit_message_text(f'Дата {result_low} выбрана верно?', call.message.chat.id, call.message.message_id)
